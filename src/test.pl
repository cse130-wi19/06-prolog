%:- initialization(working_directory(CWD,CWD)).
%:- initialization(atom_concat(CWD,'misc.pl',F)).
%:- initialization(consult(F)).
:- include('./misc.pl').
...
run_test(T,Max,Pts) :- print_list(['testing ',T,'... ']),
					   catch( ((\+(\+(T)),print('success'),Pts=Max);(print('fail'),Pts=0)), Ex, (print_exception(Ex),Pts=0) ),
					   print_list([' (',Pts,'/',Max,')\n']),!.
...
run_all_tests(Max,Pts) :- run_tests(
	...
	run_test(\+(remove_duplicates([1,2,3,4,2,3],[1,4,2,3])),1),
	run_test((remove_duplicates([1,2,3,4,2,3],X),!,X=[1,2,3,4]),1),
	run_test((union([1,2,3,4],[1,3,5,6],X),!,unordered_eq(X,[1,2,3,4,5,6])),1),
	run_test(\+((union([1,2,3],[4,3],X),!,unordered_eq(X,[1,2,3]))),1),
	run_test((intersection([1,2,3,4],[1,3,5,6],X),!,unordered_eq(X,[1,3])),1),
	run_test(\+(intersection([1,2,3],[4,3],[1])),1),
	
	run_test(\+(available_at(lengua_sopa,el_cuervo)),1),
	...
	run_test(\+(multi_available(lengua_sopa)),1),
	...
	run_test(\+(overworked(carlos)),1),
	run_test((bagof(X,overworked(X),Y),!,unordered_eq(Y,[maria,miguel])),1),
	run_test(\+(total_cost(carnitas_taco,3)),1),
	...
	run_test(\+(has_ingredients(lengua_sopa,[cheese,lengua])),1),
	run_test((bagof(X,has_ingredients(X,[salsa,guacamole,cheese]),Y),!,unordered_eq(Y,[al_pastor_taco,combo_plate])),1),
	run_test(\+(avoids_ingredients(lengua_sopa,[cheese,lengua])),1),
    ...
    ],Max, Pts), print_list(['Result :', Pts, '/', Max, '\n']) .
		

%grade :- run_all_tests(Max, Pts). %print_list(['Result: ', Pts, '/', Max, '\n']).
%:- initialization(set_prolog_flag(print_write_options, [character_escapes(true)])).
%:- initialization(run_all_tests(Max,Pts)), initialization(print_list(['Result: ',Pts,'/',Max,'\n'])).
%:- initialization(use_module(library(double_quotes))).
%:- initialization(set_prolog_flag(double_quotes, chars)).
:- initialization(set_prolog_flag(double_quotes,chars)).
:- initialization(run_all_tests(Max, Pts)).
:- initialization(halt).
